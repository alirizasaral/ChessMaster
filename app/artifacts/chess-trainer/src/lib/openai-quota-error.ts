export type VoiceCoachErrorKind = "quota" | "generic";
...
export function getVoiceCoachErrorToast(
  kind: VoiceCoachErrorKind,
  rawError?: string | null,
): { title: string; description: string } {
  if (kind === "quota") {
    return {
      title: "The coach's piggy bank is empty",
      description:
        "Your grandmaster can't listen or trash-talk right now — the voice budget ran dry. The board still works; refill the coffers and he'll be back at the mic.",
    };
  }

  return {
    title: "Couldn't start the voice coach",
    description: rawError ?? "Check your microphone permission and try again.",
  };
}
